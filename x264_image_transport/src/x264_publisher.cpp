...
namespace x264_image_transport {
    ...
    // must be called before using avcodec lib
      ...
      avformat_network_init();
			ROS_INFO("in initialize : %s",encoding.c_str());
	    //Codec
	    AVCodec *codec = 0;
      ...
      if (!encCdcCtx_)
      {
        ...
        {
            /*
            sws_ctx_ = sws_getContext(width, height, AV_PIX_FMT_BAYER_GBRG16LE, //src
                                    encCdcCtx_->width, encCdcCtx_->height, encCdcCtx_->pix_fmt, //dest
                                    SWS_FAST_BILINEAR, NULL, NULL, NULL);
            */
        ...
        avpicture_alloc((AVPicture *)encFrame_, encCdcCtx_->pix_fmt, width, height);
				//av_image_alloc(encFrame_->data, encFrame_->linesize, width, height, encCdcCtx_->pix_fmt, 32);

				ROS_INFO("(format: %d)",encCdcCtx_->pix_fmt);
        //Initialize packet
	...
	//encodedPacket_.size=height * srcstride;
		ROS_INFO("up this %d",encFrame_->linesize);
